        ...
        if (200 == $response->getStatusCode()) {
            try {
                ...
                foreach ($data->services as $channel_data) {
                    if ($channel_data->program <= 0 || '' == $channel_data->servicename)
                        continue;
        ...
        if (200 == $response->getStatusCode()) {
            try {
                ...
                foreach ($data->events as $program_data) {
                    if ('' == $program_data->title || '' == $program_data->begin_timestamp || ! in_array($program_data->sref, $channel_filter))
                        continue;
    ...
    public function load_epg(Channel $channel)
    {
        ...
        if (200 == $response->getStatusCode()) {
            try {
                ...
                DB::beginTransaction();
                foreach ($data->events as $program_data) {
                    if ('' == $program_data->title || '' == $program_data->begin_timestamp)
                        continue;

                    if (Carbon::now()->floatDiffInHours(Carbon::parse($program_data->begin_timestamp)) > $this->epg_limit)
                        continue;
        ...
        if (200 == $response->getStatusCode()) {
            try {
                ...
                foreach ($data->movies as $recording_data) {
                    if ($recording_data->eventname == 'epg.dat')
                        continue;
